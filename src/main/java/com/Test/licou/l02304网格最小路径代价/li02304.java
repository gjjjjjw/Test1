package com.Test.licou.l02304网格最小路径代价;

public class li02304 {
    ...
    public static void main(String[] args) {
        ...
        int i = new li02304().minPathCost(grid, moveCost);
    ...
    public int minPathCost(int[][] grid, int[][] moveCost) {
        ...
        int[][] lin = new int[hangNum][lieNum];
        ...
        for (int i = 1; i < hangNum; i++) {
            ...
            for (int j = 0; j < lieNum; j++) {
                ...
                int[] ints = moveCost[index];

                for (int k = 0; k < ints.length; k++) {
                    if(j == 0){
                        lin[i][k] = lin[i-1][j] + moveCost[index][k] + grid[i][k];
                    }else {
                        lin[i][k] = Math.min(lin[i][k], lin[i-1][j] + moveCost[index][k] + grid[i][k]);
                    }
        ...
        int r = lin[hangNum-1][0];
        for (int i = 1; i < lieNum; i++) {
            r = Math.min(r, lin[hangNum-1][i]);
        }
